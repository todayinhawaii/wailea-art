...
// Turns bare URLs typed as plain text (www.wailea.art, https://example.com)
// into real clickable links — but skips anything already inside an <a> tag,
// so a link someone added manually through the editor's Link button never
// gets double-wrapped.
function autoLinkUrls(html) {
  ...
  return parts.map((part, i) => {
    if (i % 2 === 1) return part; // already inside an <a>...</a> block — leave untouched
    return part.replace(urlPattern, (match) => {
      const trailingPunct = match.match(/[.,;:!?)]+$/);
      let displayText = match;
      let trailer = '';
      if (trailingPunct) {
        trailer = trailingPunct[0];
        displayText = match.slice(0, -trailer.length);
      }
      const href = /^https?:\/\//i.test(displayText) ? displayText : `https://${displayText}`;
      return `<a href="${href}" style="color:#6b8570;">${displayText}</a>${trailer}`;
    });
  }).join('');
}

// A reasonable plain-text fallback for email clients that don't render
// HTML, derived from the rich-text HTML content.
...
function wrapInWelcomeTemplate(personalMessageHtml) {
  const noteHtml = autoLinkUrls(personalMessageHtml || '<p>(Nothing written yet.)</p>');
...
module.exports = { wrapInWelcomeTemplate, autoLinkUrls, htmlToPlainText };
